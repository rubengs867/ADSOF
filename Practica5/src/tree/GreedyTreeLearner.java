...
public class GreedyTreeLearner<T, L> {
  ...
  /**
   * Estrategia a utilizar para seleccionar la mejor característica en cada
   * división del árbol.
   */
   ...
   * Inicializa el generador de árboles con una estrategia de selección de
   * características aleatoria.
   */
   ...
   * Divide el conjunto de datos en subconjuntos y crea los nodos del árbol
   * iterativamente.
   * 
   * @param arbol               El árbol de decisión que se está construyendo.
   * @param nombreNodoActual    El identificador del nodo que se está procesando
   *                            en esta llamada.
   * @param datos               La lista de objetos que han llegado hasta este
   *                            nodo.
   * @param featuresDisponibles Lista de características que aún no se han
   *                            utilizado en esta rama.
   * @param dataset             El dataset original, necesario para acceder al
   *                            Featurizer y al LabelProvider.
   * @return Un String que representa el nombre del nodo destino (o el valor de la
   *         etiqueta si es hoja).
   */
