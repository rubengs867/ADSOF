...
/**
 * Estrategia de selección de características basada en la métrica de clasificación errónea (Misclassification).
 * Esta heurística evalúa cada característica disponible agrupando los datos según los distintos
 * valores que toma dicha característica. Para cada grupo, asume que la predicción será la etiqueta
 * mayoritaria y cuenta cuántos elementos no coinciden con esa etiqueta (elementos mal clasificados).
 * La característica óptima será aquella cuya suma total de errores en todos sus grupos sea la menor.
 *
 * @param <T> El tipo de objeto que se está clasificando (ej. Person, Weather).
 * @param <L> El tipo de la etiqueta asignada a los objetos (ej. String, Boolean).
 */
public class MisclassificationStrategy<T, L> implements FeatureSelectionStrategy<T, L> {

    /**
     * Evalúa las características disponibles y elige la mejor para realizar la siguiente 
     * división en el árbol de decisión, buscando minimizar los errores de clasificación.
     *
     * @param datos La lista de objetos que han llegado hasta el nodo actual y necesitan ser divididos.
     * @param featuresDisponibles La lista de nombres de las características que aún no se han utilizado en esta rama.
     * @param dataset El conjunto de datos etiquetados, necesario para extraer los valores de las características (Featurizer) y sus etiquetas (LabelProvider).
     * @return El nombre de la característica seleccionada que produce el menor número de clasificaciones erróneas.
     */
    @Override
    public String chooseBestFeature(List<T> datos, List<String> featuresDisponibles, LabeledDataset<T, L> dataset) {
        ...
        for (String feature : featuresDisponibles) {
            ...
            // Comprobamos si esta feature es la mejor hasta ahora, la que tiene menos errores
